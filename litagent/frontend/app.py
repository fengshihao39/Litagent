...
def _render_header() -> None:
    ...
    st.caption(
        "Litagent 可以根据你的关键词和研究主题，以及正在研究的文献内容，"
        "自动检索并为你推荐相关的文献。"
    )
    st.caption(
        "在左侧设置你想要检索的文献关键词或主题，并设置文献的返回数量和时间范围，"
        "即可开始检索。"
    )
    st.caption(
        "你也可以上传自己正在研究的文献列表（支持 `.csv`、`.json`、`.bib` 和 "
        "`.bibtex` 格式），让 AI 自动分析你的研究主题。"
    )
...
def _render_results(results: list[dict]) -> None:
    ...
    for idx, paper in enumerate(results):
        with st.expander(
            ...
            safe_title = paper.get("title", "ref").replace(" ", "_")
            filename = f"{paper.get('year', 'year')}_{safe_title}.bib"
